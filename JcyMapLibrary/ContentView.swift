...
struct ContentView: View {
    ...
    @State private var drawType: DrawType?

    var body: some View {
        ZStack {
            MapView(mapState: mapState, time: time, drawType: drawType).frame(maxWidth:.infinity, maxHeight: .infinity)
            HStack(spacing: 10) {
                Button("重载") {
                ...
                Button("点面") {
                    time = Date().timeIntervalSince1970
                    mapState = 2
                    drawType = .pointToPolygon
                }
                Button("画线") {
                    time = Date().timeIntervalSince1970
                    mapState = 8
                    drawType = .drawPolyline
                }
                Button("结束绘") {
                    time = Date().timeIntervalSince1970
                    mapState = 3
